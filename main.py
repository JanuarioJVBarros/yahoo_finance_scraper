...
stock_symbol_list = [yahooquery_procedures.get_index_stocks(index_symbol) for index_symbol in stock_index]

header_list = ["Method", "Current Value", "Range", "Analysis"]
# Iterate over index and iterate over each stock
for index in stock_symbol_list:
    for ticker_symbol in index:
        ...
        row_list.append(["Intrinsic Value", current_share_value, intrincic_share_value, intrincic_share_analysis])
        ...
        for year, roe in roe_data.items():
            ...
            row_list.append([f"Return on Equity (ROE) {year}", roe, "> 15%", status])
        ...
        for year, ratio in debt_to_equity_data.items():
            ...
            row_list.append([f"Debt-to-Equity {year}", ratio, "< 0,5", status])
        ...
        for year, margin in profit_margin_data.items():
            ...
            row_list.append([f"Profit Margin {year}", margin, "> 10%", status])
        ...
        if gross_margins is not None and revenue_growth is not None and r_and_d is not None:
            for year, margin in gross_margins.items():
                ...
                row_list.append([f"Gross Margin {year}", margin, "> 40%", status])
            print("\n🔹 Revenue Growth:")
            for year, growth in revenue_growth.items():
                ...
                row_list.append([f"Revenue Growth {year}", growth, "> 5%", status])
            if r_and_d:
                print("\n🔹 R&D Spending as % of Revenue:")
                for year, rd_percent in r_and_d.items():
                    print(f"{year}: {rd_percent:.2f}%")
                    row_list.append(["R&D Spending as % of Revenue"])
                    row_list.append([f"R&D Spending as % of Revenue {year}", rd_percent, "-", "-"])
        else:
        ...
        openpyxl_procedures.write_to_excel(f"{ticker_symbol}_{datetime.strftime}", header_list, row_list)
